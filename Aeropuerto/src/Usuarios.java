...
public class Usuarios {
	...
	{
		for(int i = 0; i < registroUsuarios.length; i++) {
			System.out.println("correo: "+registroUsuarios[i][0]+" contraseña: "+registroUsuarios[i][1]+" nombre: "+registroUsuarios[i][2]);
		}
	}
	...
	public boolean ValidarUser(String correo, String pass) {
		...
		for(int i = 0; i < registroUsuarios.length; i++) {
			...
			if(correo.equals(registroUsuarios[i][0]) && pass.equals(registroUsuarios[i][1])) {
				val = true;
			}else {
				val = false;
			}
